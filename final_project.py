...
mask_paths = [f'mask{i}.png' for i in range(1, 6)]  # Названия файлов mask1.png, mask2.png и так далее
masks = [cv2.imread(path, cv2.IMREAD_UNCHANGED) for path in mask_paths]

image_path = "inst.png"  # Путь к изображению
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    ...
    # Накладываем маску (тут что-то с альфа-каналом, с альфа каналом помог gpt)
            ...
            if not ret:
                # Если видео закончилось, возвращаемся к первому кадру
                video_cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                continue

            # Показываем видео
            cv2.imshow("Video View", video_frame)
